...
`dra` 는 `dooray-agent` 의 짧은 별칭이다. 로컬 MCP 서버 bin `dooray-agent-mcp` 도 함께 설치된다.
...
npm install
npm run build
npm link      # 전역에 dra / dooray-agent / dooray-agent-mcp 링크
```
...
토큰은 `~/.dooray-agent/config.json` 에 `0600` 로 로컬 저장된다(외부 전송 없음).

> **메일은 선택 기능** — `mail *` 은 Dooray REST 토큰과 **별개로** IMAP/SMTP 설정이 필요하다:
> `dra config set imap-host imap.dooray.com` · `mail-user <주소>` · `mail-password <IMAP 앱 비밀번호>`(발송은 `smtp-host` 추가).
> 미설정이어도 다른 명령은 정상 동작하고, 메일 명령만 설정 방법을 안내한다.
