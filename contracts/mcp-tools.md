...
반환 열: **read** = API 원자료(해당 리소스 타입, `src/dooray/types.ts`) · **write** = `{ ...id, status }` 봉투.
...
- **반환**: `src/dooray/types.ts` 리소스 타입(읽기) + `{ ...id, status }` 쓰기 봉투(`contracts/api-spec.md` 「쓰기 결과 status 값」)를 **그대로 재사용**. 신규 타입 SSOT 없음 — `tsc` 가 드리프트를 잡는다.
...
- messenger·drive·calendar·mail·admin/config/setup/feedback/capabilities.
