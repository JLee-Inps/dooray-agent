...
CLI 가 없으면 먼저 설치한다. npm 레지스트리 미배포 시 소스 설치: `git clone https://github.com/JLee-Inps/dooray-agent.git && cd dooray-agent && pnpm install && pnpm build && npm link`
...
- **제공 툴**: 27개 (`dooray_whoami`, `dooray_post_*`, `dooray_wiki_*`, `dooray_member_search`, `dooray_project_list`, `dooray_calendar_*` 6개, `dooray_mail_*` 3개 — 메일 툴은 IMAP/SMTP 설정 별도 필요)
